...
const App = () => {
  ...
  const exportCard = () => {
    ...
    // Set for high quality (300 DPI equivalent)
  ...
  const BusinessCard = () => {
    ...
    return (
      <svg 
        ...
        className="w-full h-auto shadow-2xl rounded-sm transition-all duration-300"
          ...
          <g>
            ...
            {cardData.qrEnabled && (
              <g transform={`translate(${width - 200}, ${height - 200})`}>
                <rect width="120" height="120" fill="white" rx="10" filter="drop-shadow(0 4px 6px rgba(0,0,0,0.1))" />
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-900 text-slate-200 overflow-hidden">
      ...
      <header className="h-16 bg-slate-800 border-b border-slate-700 flex items-center justify-between px-6 shrink-0">
        ...
        <button 
          onClick={exportCard}
          className="bg-blue-600 hover:bg-blue-500 text-white px-5 py-2 rounded-lg font-bold flex items-center gap-2 transition-all active:scale-95"
        >
      ...
      <div className="flex flex-1 overflow-hidden">
                  ...
                  {Object.entries(palette).map(([key, value]) => (
                    <div key={key} className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-mono opacity-50 uppercase">{value}</span>
                        <input 
                          type="color" 
                          value={value} 
              ...
              <section>
                ...
                <div className="space-y-3">
                  <label className="block p-4 border-2 border-dashed border-slate-700 rounded-xl hover:border-blue-500 transition-colors cursor-pointer text-center group">
                    ...
                    <Upload size={20} className="mx-auto mb-2 text-slate-500 group-hover:text-blue-500" />
        ...
        {/* Main Canvas Area */}
        <main className="flex-1 bg-slate-950 flex flex-col items-center justify-center p-12 relative">
          <div className="absolute top-8 left-1/2 -translate-x-1/2 flex bg-slate-800 p-1 rounded-xl shadow-2xl border border-slate-700">
            <button 
          ...
          <div className="w-full max-w-4xl relative group">
            ...
            <div className="absolute -right-16 top-0 flex flex-col gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
              <button className="p-3 bg-slate-800 rounded-full text-slate-400 hover:text-white hover:bg-slate-700 shadow-xl" title="Zoom In">
                <Maximize2 size={20} />
              </button>
              <button className="p-3 bg-slate-800 rounded-full text-slate-400 hover:text-white hover:bg-slate-700 shadow-xl" title="Reset View">
                <RotateCcw size={20} />
              </button>
            </div>
          </div>
        ...
        {/* Right Asset Explorer (Optional/Modern UI) */}
        <aside className="w-16 bg-slate-800 border-l border-slate-700 flex flex-col items-center py-6 gap-6">
          <button className="p-2 text-blue-400 bg-blue-400/10 rounded-lg"><Layers size={20} /></button>
          <button className="p-2 text-slate-500 hover:text-slate-200 transition-colors"><Type size={20} /></button>
          <button className="p-2 text-slate-500 hover:text-slate-200 transition-colors"><ImageIcon size={20} /></button>
          <div className="mt-auto">
             <button className="p-2 text-slate-500 hover:text-white transition-colors"><Share2 size={20} /></button>
          </div>
        </aside>
      </div>
      ...
      <footer className="h-10 bg-slate-900 border-t border-slate-800 flex items-center justify-between px-6 text-[10px] text-slate-500 font-medium uppercase tracking-[0.2em]">
        ...
        <div className="flex items-center gap-4">
          <span>Project ID: NC-{Math.random().toString(36).substr(2, 9).toUpperCase()}</span>
